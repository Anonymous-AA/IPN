...
class IPNX(nn.Module):
  """Isometric Propagation Network"""
  def __init__(self, att_dim, image_dim, att_hC, hidden_C, T, degree, tau):
    super(IPNX, self).__init__()
  ...
  def extra_repr(self):
    ...
    return ('{name}(degree={degree:}, thresh={thresh:.3f}, temperature={T:}, tau={tau:}, '.format(name=self.__class__.__name__, **self.__dict__) + xshape + ')')
  ...
  def _propagate(self, img_proto, attributes):
    for _step in range(self.tau):
      ...
      img_proto, attributes = img_proto_outs, att_outs
    if self.tau == 0:
      att_outs      , (raw_att_att, attention_att) = self.get_new_attribute(attributes)
      img_proto_outs, (raw_att_img, attention_img) = self.get_new_img_proto(img_proto) 
      att_outs, img_proto_outs = attributes, img_proto
